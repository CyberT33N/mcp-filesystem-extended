...
`list_directory_entries` belongs to the discovery family and follows the global public-limit-disclosure policy with a discovery-specific emphasis.
...
For `list_directory_entries`, that includes:
...
Stable operation-wide delivery rules belong in the runtime tool description because they shape caller planning for the whole result surface.
