const SERVER_INSTRUCTION_LINES = [
  ...
  "- Traversal-oriented discovery and recursive inspection tools exclude a server-owned default baseline of vendor/cache/build trees when callers provide broad roots.",
  ...
  "- `respectGitIgnore` is an additive opt-in that can layer directory-scoped hierarchical `.gitignore` exclusions on top of the server-owned default traversal baseline.",
  ...
  "- Recursive inspection admission is parameter-aware: strong narrowing signals such as include globs should shape traversal cost before preview or completion lanes are selected, including preflight entry-budget accounting for broad recursive workloads.",
  ...
  "- For preview-capable families, `resumeMode = 'complete-result'` keeps the server-owned completion contract: the shared global response fuse remains the caller-visible final ceiling, while bounded preview-lane soft-time budgets do not govern the completion pass.",
  ...
  "- When recursive search preflight stops early and repository-local `.gitignore` refinement is available but inactive, the response may advise retrying with `respectGitIgnore = true` as an optional narrowing refinement rather than treating `.gitignore` as the default contract.",
