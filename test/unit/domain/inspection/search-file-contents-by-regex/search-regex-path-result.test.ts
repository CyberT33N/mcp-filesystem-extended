...
describe("getSearchRegexPathResult", () => {
  ...
  it("persists a materialized completion plan when a manifest-backed batch truncates early", async () => {
    const sandboxRootPath = await mkdtemp(
      join(tmpdir(), "mcp-fs-regex-complete-result-plan-"),
    );
    const alphaFilePath = join(sandboxRootPath, "alpha.ts");
    const betaFilePath = join(sandboxRootPath, "nested", "beta.ts");
    const executionPolicy = {
      ...resolveSearchExecutionPolicy(DEFAULT_CONSERVATIVE_IO_CAPABILITY_PROFILE),
      traversalInlineEntryBudget: 0,
      traversalInlineDirectoryBudget: 0,
    ...
    try {
      ...
      queueNativeStreamingBatchResult(`${alphaFilePath}:1:export const needle = true;`);
      ...
      expect(result.nextContinuationState?.materializedExecutionPlan).not.toBeNull();
      expect(result.nextContinuationState?.materializedExecutionPlan?.nextUnitIndex).toBe(1);
    } finally {
      await rm(sandboxRootPath, { recursive: true, force: true });
    }
  });
  ...
  it("delivers canonical content exactly once and attributes an alias registered before the deferred batch delivery", async () => {
    const sandboxRootPath = await mkdtemp(join(tmpdir(), "mcp-fs-regex-alias-after-"));
  ...
  it("reports an alias encountered after an immediately delivered match as already delivered in the same pass", async () => {
    const sandboxRootPath = await mkdtemp(join(tmpdir(), "mcp-fs-regex-alias-event-"));
    const executionPolicy = {
      ...resolveSearchExecutionPolicy(DEFAULT_CONSERVATIVE_IO_CAPABILITY_PROFILE),
      traversalInlineEntryBudget: 0,
      traversalInlineDirectoryBudget: 0,
  ...
  it("attaches alias attribution to the canonical match when the alias came first", async () => {
    const sandboxRootPath = await mkdtemp(join(tmpdir(), "mcp-fs-regex-alias-first-"));
  ...
  it("reports aliases whose targets escape the requested root as outside-scope events", async () => {
    const sandboxRootPath = await mkdtemp(join(tmpdir(), "mcp-fs-regex-alias-scope-"));
    const outsideRootPath = await mkdtemp(join(tmpdir(), "mcp-fs-regex-alias-outside-"));
  ...
  it("terminates cleanly when a directory alias points back into its own tree", async () => {
    const sandboxRootPath = await mkdtemp(join(tmpdir(), "mcp-fs-regex-alias-loop-"));
  ...
  it("delivers canonical content exactly once through the materialized complete-result plan", async () => {
    const sandboxRootPath = await mkdtemp(join(tmpdir(), "mcp-fs-regex-alias-plan-"));
    const executionPolicy = {
      ...resolveSearchExecutionPolicy(DEFAULT_CONSERVATIVE_IO_CAPABILITY_PROFILE),
      traversalInlineEntryBudget: 0,
      traversalInlineDirectoryBudget: 0,
  ...
  it("leaves batch matches without attribution when the backend reports an unmapped file", async () => {
    const sandboxRootPath = await mkdtemp(join(tmpdir(), "mcp-fs-regex-alias-unmapped-"));
  ...
  it("reports an alias encountered in a later pass as already delivered without re-delivering the match", async () => {
    const sandboxRootPath = await mkdtemp(join(tmpdir(), "mcp-fs-regex-alias-resume-"));
    const executionPolicy = {
      ...resolveSearchExecutionPolicy(DEFAULT_CONSERVATIVE_IO_CAPABILITY_PROFILE),
      traversalInlineEntryBudget: 0,
      traversalInlineDirectoryBudget: 0,
  ...
  it("attributes aliases on decoded-fallback units inside the materialized completion plan", async () => {
    const sandboxRootPath = await mkdtemp(join(tmpdir(), "mcp-fs-regex-alias-utf16-plan-"));
    const executionPolicy = {
      ...resolveSearchExecutionPolicy(DEFAULT_CONSERVATIVE_IO_CAPABILITY_PROFILE),
      traversalInlineEntryBudget: 0,
      traversalInlineDirectoryBudget: 0,
  ...
  it("attributes aliases through the inline decoded-fallback branch of the traversal loop", async () => {
    const sandboxRootPath = await mkdtemp(join(tmpdir(), "mcp-fs-regex-alias-utf16-inline-"));
  ...
  it("fires the already-delivered event inside the materialization loop when a resumed completion pass meets the alias", async () => {
    const sandboxRootPath = await mkdtemp(join(tmpdir(), "mcp-fs-regex-alias-materialize-event-"));
    const executionPolicy = {
      ...resolveSearchExecutionPolicy(DEFAULT_CONSERVATIVE_IO_CAPABILITY_PROFILE),
      traversalInlineEntryBudget: 0,
      traversalInlineDirectoryBudget: 0,
  ...
  it("enforces maxResults as a true total across files and resumes without loss across passes", async () => {
    const sandboxRootPath = await mkdtemp(join(tmpdir(), "mcp-fs-regex-total-budget-"));
    const executionPolicy = {
      ...resolveSearchExecutionPolicy(DEFAULT_CONSERVATIVE_IO_CAPABILITY_PROFILE),
      traversalInlineEntryBudget: 0,
      traversalInlineDirectoryBudget: 0,
    ...
    try {
      ...
      let continuationState: Parameters<typeof getSearchRegexPathResult>[0]["continuationState"] = null;
      let isFirstPass = true;

      while (isFirstPass || continuationState !== null) {
        const passResult = await getSearchRegexPathResult({
          toolName: SEARCH_FILE_CONTENTS_BY_REGEX_TOOL_NAME,
          searchPath: sandboxRootPath,
          pattern: "needle",
          filePatterns: [],
  ...
  it("keeps the per-file max-count flag on the single-file lane", async () => {
    const sandboxRootPath = await mkdtemp(join(tmpdir(), "mcp-fs-regex-single-file-maxcount-"));
  ...
  it("returns an unstopped empty result when the batch backend reports no matches", async () => {
    const sandboxRootPath = await mkdtemp(join(tmpdir(), "mcp-fs-regex-batch-empty-"));
  ...
  it("propagates native batch failures without converting them into truncation", async () => {
    const sandboxRootPath = await mkdtemp(join(tmpdir(), "mcp-fs-regex-batch-failure-"));
  ...
  it("runs no blocking admission probe on a resume pass", async () => {
    const sandboxRootPath = await mkdtemp(join(tmpdir(), "mcp-fs-regex-resume-no-probe-"));
    const executionPolicy = {
      ...resolveSearchExecutionPolicy(DEFAULT_CONSERVATIVE_IO_CAPABILITY_PROFILE),
      traversalInlineEntryBudget: 0,
      traversalInlineDirectoryBudget: 0,
  ...
  it("skips structurally invalid native output lines inside the streaming batch", async () => {
    const sandboxRootPath = await mkdtemp(join(tmpdir(), "mcp-fs-regex-malformed-line-"));
  ...
  it("falls back to the exit-code message when the native batch fails without stderr", async () => {
    const sandboxRootPath = await mkdtemp(join(tmpdir(), "mcp-fs-regex-empty-stderr-"));
